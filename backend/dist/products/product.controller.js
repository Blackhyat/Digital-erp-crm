...
import { productSchema, updateProductSchema, stockMovementSchema, } from "./product.validation.js";
...
export const getProducts = async (req, res) => {
    try {
        ...
        const conditions = [];
        if (search) {
            conditions.push(or(ilike(products.name, `%${search}%`), ilike(products.sku, `%${search}%`), ilike(products.category, `%${search}%`)));
        }
        if (category) {
